...
def train(local_rank, args):
    ...
    if args["mode"] == "clr":
        if args["deit"]:
            model = PatchCLR(backbone, 768, config.layer_norm_eps, patchclr_w=1.0, simclr_w=0.5, clustering_w=0.0,
            ...
            try:
                if no_sync and (step + 1) % iter_size != 0:
                    ...
                    if extra_negative_repr_simclr is not None:
                        start = max(extra_negative_repr_simclr.size(0) - (max(256 // args["world_size"], iter_size) * bs_size[0]), 0)
