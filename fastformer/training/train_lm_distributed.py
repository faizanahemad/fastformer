...
def build_dataloader(location, shuffle_dataset, sampling_fraction, config, collate_fn, tokenizer, continuous_iter=True, world_size=1, num_workers=1):
    ...
    try:
        ...
        #     v.training = False
        train_loader = {k: DataLoader(v, sampler=None if single_node else DistributedSampler(v, shuffle=shuffle_dataset, ), batch_size=12, collate_fn=collate_fn, prefetch_factor=2 if num_workers > 0 else None, num_workers=(2*num_workers) if single_node else num_workers) for k, v in train_dataset.items()}
