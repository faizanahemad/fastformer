    ...
    def __call__(self):
        ...
        for idx, (k, dataset) in enumerate(sorted(datadict.items())):
            ...
            with open('validation.txt', 'a') as f:
                print(str(dict(k=results[k])) + os.linesep, file=f)
...
def train(local_rank, args):
    ...
    ddp_model = DDP(model, device_ids=None if args["cpu"] else [gpu_device], find_unused_parameters=False, bucket_cap_mb=128)  # find_unused_parameters=True
    try:
