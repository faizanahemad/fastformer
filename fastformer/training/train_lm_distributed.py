...
def build_dataloader(location, shuffle_dataset, sampling_fraction, config, collate_fn, tokenizer, single_node=False, continuous_iter=True):
    try:
        ...
        #     v.training = False
        train_loader = {k: DataLoader(v, sampler=None if single_node else DistributedSampler(v, shuffle=shuffle_dataset, ), batch_size=12, collate_fn=collate_fn, prefetch_factor=4 if single_node else 2, num_workers=2 if single_node else 1) for k, v in train_dataset.items()}
