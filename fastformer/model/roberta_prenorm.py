...
class RobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        if num_layers is not None and num_layers < total_layers:
            ...
            probas = torch.tensor([((total_layers - i) / total_layers) if (i >= start_sampling_from and i not in exclude_layers) else 0.0 for i in range(total_layers)]) ** max(self.sampling_alpha, 0.01)
        ...
        layer_scales = dict(zip(self.scale_factors_idx, torch.abs(self.scale_factors_emb)))
    ...
    def forward(
        ...
        with torch.set_grad_enabled(not no_grad_embedding and self.training and start_sampling_from == 0 and torch.is_grad_enabled()):
            embedding_output = self.embeddings(
                ...
                layer_normalizer=self.layer_normalizers[0] if self.layer_normalizers is not None else None,
            )
