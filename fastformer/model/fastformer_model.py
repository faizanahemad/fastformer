...
class FastFormerForFusedELECTRAPretraining(FastFormerPreTrainedModel):
    ...
    def forward(
        ...
        if self.sentence_order_prediction_w > 0 and labels_segment_index is not None:
            ...
            print("[FastFormerForFusedELECTRAPretraining]: Time = %s, Logits and Labels SOP = %s" % (get_time_string(), list(zip(sent_order_logits.detach().view(-1, (self.cls_tokens + 1)), labels_segment_index.view(-1)))[:4]))
            if record_accuracy:
        ...
        print("[FastFormerForFusedELECTRAPretraining]: Time = %s, Logits and Labels for electra = %s" % (get_time_string(), list(zip(active_logits.detach(), labels))[:4]))
